...
class Funcs():      #FUNÇÕES DO BACKEND
    ...
    def OnDoubleClick(self, event):  # Duplo Click
        ...
        for n in self.listaCli.selection():
            col1, col2, col3, col4, col5, col6, col7, col8, col9, col10, c0l11= self.listaCli.item(n, 'values')
    ...
    def selecionar_foto():
        ...
        filename = filedialog.askopenfilename(initialdir='/', title='Selecionar Foto', filetypes=(('Arquivos de     Imagem', '*.png *.jpg *.jpeg *.gif'), ('Todos os arquivos', '*.*')))
        if filename:
            imagem = Image.open(filename)
            imagem_resized = imagem.resize((150, 150))
            foto = ImageTk.PhotoImage(imagem_resized)
            label_foto.config(image=foto)
            label_foto.image = foto
            foto_blob = sqlite3.Binary(imagem.tobytes())    
...
class Application(Funcs, Relatorios):   #FUNÇÕES DO FRONTEND
    ...
    def frames_da_tela(self):
        ...
        self.frame_imagem.place(relx=0.01, rely=0.01, relwidth=0.23, relheight=0.80)
    ...
    def widgets_frame1(self):
       ...
       ###CRIAÇÃO DO BOTÃO NOVO####  
        ...
        # BOTÃO FOTO
        self.btn_selecionar_foto = Button(self.frame_imagem, text='Selecionar Foto', font=('Bold, 12'), command=self.selecionar_foto)
        self.btn_selecionar_foto.place(relx=0.1, rely=0.4, relwidth=0.45, relheight=0.10)

        self.label_foto = Label(self.frame_imagem)
        self.label_foto.place(relx=0.1, rely=0.01, relwidth=0.70, relheight=0.30)
              
        ###CRIAÇÃO DA LABEL E ENTRY DDO CODIGO
